...
export default abstract class BaseJob {
    ...
    private initializeJob(): void {
        ...
        try {
            ...
            console.error(
                utils.consoleStyles(`✗ Failed to initialize job "${this.name}": Invalid schedule "${this.schedule}"`, [
                    "red",
                    "bold",
                ]),
            );
    ...
    public async runNow(): Promise<void> {
        ...
        try {
            await this.execute();
            this.log(
                utils.consoleStyles(`✓ ${this.successMessage || `Job "${this.name}" completed successfully`}`, [
                    "green",
                ]),
            );
        } catch (error) {
