...
import { INotificationJob } from "../../interfaces/NotificationJob";
...
export default class NotificationDispatchJob extends BaseJob {
    ...
    private async processJob(job: INotificationJob, counters: DispatchCounters): Promise<void> {
        ...
        await NotificationDispatchService.markFailed(job.id, result.error || "Provider request failed", result.statusCode);
    ...
    protected async execute(): Promise<void> {
        ...
        try {
            ...
            if (now - this.lastStaleSweepAt >= STALE_SWEEP_INTERVAL_MS) {
                const reclaimed = await NotificationDispatchService.requeueStaleProcessingJobs(STALE_PROCESSING_TIMEOUT_MS);
                if (reclaimed > 0) {
