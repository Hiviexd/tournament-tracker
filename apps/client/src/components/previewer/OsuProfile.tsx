...
export default function OsuProfile({ user, onDeleteBadge }: OsuProfileProps) {
    ...
    const sortedBadges = user.badges
        ? user.badges.toSorted((a, b) => dayjs(b.awarded_at).valueOf() - dayjs(a.awarded_at).valueOf())
        : [];
                ...
                <div className="profile-details">
                    <div className="username-container">
                        ...
                        <div className="user-badges">
                            ...
                            {user.groups &&
                                user.groups.map((group) => <OsuUserGroupBadge key={group.id} group={group} />)}
                        </div>
                    </div>
                    {user.title && (
                        <div
                            className="user-title"
                            style={{ color: utils.sanitizeCssColor(user.profile_colour) ?? undefined }}>
                            {user.title}
                        </div>
                    )}
                    <Box mt="5">
